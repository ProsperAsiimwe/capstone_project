...
class ApplicantRelevantQualificationController {
  /**
   * GET All applicantRelevantQualifications.
   *
   * @param {*} req Request
   * @param {*} res Response
   *
    ...
    try {
      const result =
        await applicantRelevantQualificationService.findAllApplicantRelevantQualifications(
          {
            ...getApplicantRelevantQualificationAttributes(),
          }
        );

      http.setSuccess(
        200,
        'Applicant Relevant Qualifications Fetched Successfully',
        {
          data: result,
        }
      );
      ...
      http.setError(400, 'Unable To Fetch Applicant Relevant Qualifications', {
  ...
  /**
   * CREATE New ApplicantRelevantQualification Data.
   *
   ...
   */
  async createApplicantRelevantQualification(req, res) {
    try {
      const data = req.body;
      const { id } = req.user;
      ...
      const response = await model.sequelize.transaction(
        async (transaction) => {
          ...
          const result =
            await applicantRelevantQualificationService.createApplicantRelevantQualification(
              newData,
              transaction
            );
      ...
      http.setError(
        400,
        'Unable to create this Applicant Relevant Qualification.'
      );
  ...
  /**
   * UPDATE Specific ApplicantRelevantQualification Data.
   *
   * @param {*} req Request body
   * @param {*} res Response
   *
   * @return {JSON} Return JSON Response
   */
  async updateApplicantRelevantQualification(req, res) {
    try {
      const { id } = req.params;
      const data = req.body;

      const findForm =
        await applicantRelevantQualificationService.findOneApplicantRelevantQualification(
          {
            where: {
              id,
            },
      ...
      const updateResponse =
        await applicantRelevantQualificationService.updateApplicantRelevantQualification(
          id,
          data
        );
  ...
  /**
   * DELETE Specific ApplicantRelevantQualification Data.
   *
   * @param {*} req Request body
   * @param {*} res Response
   *
   * @return {JSON} Return JSON Response
   */
  async deleteApplicantRelevantQualification(req, res) {
    try {
      const { id } = req.params;
      const applicantId = req.user.id;

      const deleteResponse =
        await applicantRelevantQualificationService.deleteApplicantRelevantQualification(
  ...
  /**
   * Get Specific ApplicantRelevantQualification Data.
   *
   * @param {*} req Request body
   * @param {*} res Response
   *
   * @return {JSON} Return JSON Response
   */
  async fetchApplicantRelevantQualification(req, res) {
    try {
      ...
      http.setSuccess(
        200,
        'Applicant Relevant Qualification fetch successful',
        {
          data: result,
        }
      );
      ...
      http.setError(
        400,
        'Unable to get this Applicant Relevant Qualification',
        {
          error: { message: error.message },
        }
      );
...
const getApplicantRelevantQualificationAttributes = function () {
  return {
    // include: [
    //   {
    //     association: 'applicant',
    //     attributes: { exclude: ['remember_token', 'password'] },
    //   },
    // ],
  };
};

module.exports = ApplicantRelevantQualificationController;
