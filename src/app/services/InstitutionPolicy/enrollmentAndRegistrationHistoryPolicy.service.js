...
class enrollmentAndRegistrationHistoryPolicyService {
  /**
  ...
  static async findAllRecords(options) {
    try {
      const records =
        await models.EnrollmentAndRegistrationHistoryPolicy.findAll({
          ...options,
        });
      ...
      await sequelizeErrorHandler(
        error,
        `enrollmentAndRegistrationHistoryPolicy.service.js`,
        `findAllRecords`,
        `GET`
      );
    }
  ...
  static async findOneRecord(options) {
    try {
      const record =
        await models.EnrollmentAndRegistrationHistoryPolicy.findOne({
          ...options,
        });
  ...
  /** createEnrollmentAndRegistrationHistoryPolicyRecord
   *
   * @param {*} data
   * @param {*} transaction
   */
  static async createRecord(data, transaction) {
    try {
      const record =
        await models.EnrollmentAndRegistrationHistoryPolicy.findOrCreate({
          where: {
  ...
  static async updateRecord(id, data, transaction) {
    try {
      const record = await models.EnrollmentAndRegistrationHistoryPolicy.update(
        { ...data },
  ...
  static async deleteRecord(id, transaction) {
    try {
      const deleted =
        await models.EnrollmentAndRegistrationHistoryPolicy.destroy({
          where: { id },
          transaction,
        });
...
module.exports = enrollmentAndRegistrationHistoryPolicyService;
